...
public class PlayerHealthSplit2 : MonoBehaviour
{
    public CollisionSplit1 collisionScript; // nastavy se CollisionSplit1

    public GameManager2 gameManager; // nastavy se gameManager2

    public Animator animator; // Animator komponenta hráèe

    public CollisionSplit1 aiCollision; // nastavy se CollisionSplit1
    ...
    void Start()
    {
        ...
        collisionScript = GetComponent<CollisionSplit1>();
    ...
    private IEnumerator koTime(float koSeconds, float gettingUpSeconds, float reviveTime)
    {
        ...
        if (playerGetkoCount == 1)
        {
            ...
            {
                gameManager.pressNum0Text.enabled = true;
                // Zde sledujeme stisknutí mezerníku
                if (Input.GetKeyDown(KeyCode.Keypad0))
                {
                    spacebarCount++;
                    Debug.Log("Numpad0 stisknut: " + spacebarCount);
            ...
            if (spacebarCount >= numberOfPressedSpaceToGetUp)
            {
                ...
                gameManager.pressNum0Text.enabled = false;
                ...
                gameManager.damageOverlay1.SetActive(true);
                gameManager.damageOverlay2.SetActive(true);
            }
