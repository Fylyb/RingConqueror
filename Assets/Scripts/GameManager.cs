...
public class GameManager : MonoBehaviour
{
    ...
    public Collision collisionPlayerScript;
    public AIcollision collisionAIScript;
    ...
    private float _roundTime = 30f; // 2 minuty  20f = 20 sekund
    ...
    void Start()
    {
        ...
        aiHealth = opponentHitbox.GetComponent<AIhealth>();

        _playerStartStamina = playerStaminaManager.maxStamina;
        //aiStartStamina = aiOponentStaminaManager.maxStamina;
    ...
    IEnumerator AiNeutralCorner()
    {
        ...
        Vector3 target = new Vector3(/*5.27f, 1.03f,*/-5.54f, 1.03f, 4.99f);
    ...
    void PlayerNeutralCornera()
    {
        //yield return new WaitForSeconds(1.5f);
        Debug.Log("vola sa");
        Vector3 target1 = new Vector3(/*-5.34f, 1.03f,*/4.8f, 1.1f, -5.57f);
    ...
    void HandleRoundResult()
    {
        ...
        if (collisionPlayerScript.hitPlayer > collisionAIScript.hitAI)
            _playerScore++;
        else if (collisionPlayerScript.hitPlayer < collisionAIScript.hitAI)
            _aiScore++;

        Debug.Log("player hits " + collisionPlayerScript.hitPlayer);
        Debug.Log("ai hits " + collisionAIScript.hitAI);

        // Aktualizuje skóre po každém kole
        UpdateScore();
        playerHealth.HealPlayer();
        aiHealth.HealOpponent();
    }
    ...
    public void NextRound()
    {
        if (_currentRound < _totalRounds)
        {
            ...
            aiHealth.aiHealthSlider.value = aiHealth.currentHealth / aiHealth.maxHealth;
